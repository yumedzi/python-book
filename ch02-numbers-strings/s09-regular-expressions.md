...
To make these quantifiers (\`\*\` and \`+\`) lazy (non-greedy) to much as few as possible we can add \`?\` to them. Now the example from above correctly returns the contents of all \`\` tags:
